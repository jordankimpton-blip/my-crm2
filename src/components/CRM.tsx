...
export default function CRM() {
  ...
  async function handleAddGlobalAction() {
    if (!actionForm.text.trim()) { setActionError("Action text is required."); return; }
    await addAction({ text:actionForm.text.trim(), contact_id:actionForm.contactId ? Number(actionForm.contactId) : null, due_date:actionForm.dueDate||null, done:false });
    setActionForm({ text:"", contactId:"", dueDate:"" }); setActionError(""); setAddingAction(false);
  }

  async function handleAddFollowUp(contactId: number) {
    if (!followUpForm.text.trim()) { setFollowUpError("Please enter an action."); return; }
    await addAction({ text:followUpForm.text.trim(), contact_id:contactId, due_date:followUpForm.dueDate||null, done:false });
...
${actions.filter(a=>!a.done).map(a=>`- ${a.text}${a.due_date?` (due ${a.due_date})`:""}${a.contact_id?` [re: ${contacts.find(c=>c.id===a.contact_id)?.name||"unknown"}]`:""}`).join("\n")||"None."}`;
  ...
  // ── Styles ──
  const S = {
    page:       { fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", maxWidth:640, margin:"0 auto", padding:"1.5rem 1rem" },
    topBar:     { display:"flex" as const, alignItems:"center", justifyContent:"space-between", marginBottom:"1.5rem", gap:8, flexWrap:"wrap" as const },
    title:      { fontSize:20, fontWeight:500, color:"#111", margin:0 },
    btn:        { padding:"7px 16px", borderRadius:8, border:"0.5px solid #d1d5db", background:"transparent", color:"#111", cursor:"pointer", fontSize:13, fontWeight:500 },
    btnPrimary: { padding:"7px 16px", borderRadius:8, border:"none", background:"#111", color:"#fff", cursor:"pointer", fontSize:13, fontWeight:500 },
    btnSm:      { padding:"4px 10px", borderRadius:8, border:"0.5px solid #e5e7eb", background:"transparent", color:"#6b7280", cursor:"pointer", fontSize:12 },
    input:      { width:"100%", padding:"8px 12px", borderRadius:8, border:"0.5px solid #d1d5db", background:"#fff", color:"#111", fontSize:14, boxSizing:"border-box" as const },
    label:      { fontSize:12, color:"#6b7280", marginBottom:4, display:"block" },
    card:       { background:"#fff", border:"0.5px solid #e5e7eb", borderRadius:12, padding:"12px 16px", cursor:"pointer", marginBottom:8, display:"flex", alignItems:"center", gap:12, transition:"border-color 0.15s" },
    field:      { marginBottom:16 },
    section:    { fontSize:11, fontWeight:500, color:"#6b7280", textTransform:"uppercase" as const, letterSpacing:"0.06em", margin:"20px 0 8px" },
    alert:      { background:"#FAEEDA", border:"0.5px solid #EF9F27", borderRadius:8, padding:"10px 14px", marginBottom:14, display:"flex", alignItems:"center", gap:8, cursor:"pointer" },
    divider:    { borderTop:"0.5px solid #e5e7eb", margin:"16px 0" },
  };

  function NavBtn({ id, label, badge }: { id: string; label: string; badge: number }) {
    const active = view === id;
    return (
      <button onClick={()=>setView(id)} style={{ padding:"6px 14px", borderRadius:8, border:"0.5px solid #d1d5db", background:active?"#f3f4f6":"transparent", color:active?"#111":"#6b7280", cursor:"pointer", fontSize:13, fontWeight:active?500:400, display:"flex", alignItems:"center", gap:5 }}>
        {label}
  ...
  if (loading) return <div style={{ ...S.page, paddingTop:"3rem", textAlign:"center", color:"#6b7280" }}>Loading…</div>;
    ...
    <div style={{ ...S.page, display:"flex", flexDirection:"column", minHeight:500 }}>
      <div style={S.topBar}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {aiMessages.length > 0 && <button style={S.btn} onClick={() => setAiMessages([])}>Clear</button>}
          <button style={S.btn} onClick={() => setView("list")}>← Back</button>
        </div>
      </div>
      {contacts.length === 0 && <p style={{ fontSize:13, color:"#6b7280" }}>Add some contacts first.</p>}
      <div style={{ flex:1, overflowY:"auto", marginBottom:16, display:"flex", flexDirection:"column", gap:12 }}>
        ...
        {aiMessages.map((m,i) => (
          <div key={i} style={{ alignSelf:m.role==="user"?"flex-end":"flex-start", maxWidth:"85%", background:m.role==="user"?"#f3f4f6":"#fff", border:"0.5px solid #e5e7eb", borderRadius:12, padding:"10px 14px", fontSize:14, lineHeight:1.6, color:"#111", whiteSpace:"pre-wrap" }}>{m.content}</div>
        ))}
        {aiLoading && <div style={{ alignSelf:"flex-start", fontSize:13, color:"#6b7280", padding:"8px 0" }}>Thinking…</div>}
        <div ref={aiEndRef} />
      </div>
      <div style={{ display:"flex", gap:8 }}>
  ...
  if (view === "actions") return (
    <div style={S.page}>
      ...
      {alertActions.length > 0 && <div style={S.alert}><span style={{ fontSize:15 }}>⚠</span><span style={{ fontSize:13, color:"#633806", fontWeight:500 }}>{alertActions.length} item{alertActions.length>1?"s":""} overdue or due today</span></div>}
      {addingAction && (
        <div style={{ background:"#f9fafb", border:"0.5px solid #d1d5db", borderRadius:12, padding:"14px 16px", marginBottom:16 }}>
      ...
      {openActions.length > 0 && (<>
        ...
        {openActions.map(a => {
          ...
          return (
            <div key={a.id} style={{ background:"#fff", border:"0.5px solid #e5e7eb", borderRadius:8, padding:"10px 14px", marginBottom:8, display:"flex", alignItems:"flex-start", gap:10 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ margin:0, fontSize:13, color:"#111", lineHeight:1.5 }}>{a.text}</p>
                <div style={{ display:"flex", gap:8, alignItems:"center", marginTop:5, flexWrap:"wrap" }}>
                  {linked && <span onClick={() => { setSelected(linked); setDetailTab("followup"); setView("detail"); }} style={{ fontSize:11, color:"#185FA5", cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}><Avatar name={linked.name} size={14} /> {linked.name}</span>}
      ...
      {doneActions.length > 0 && (<>
        ...
        {doneActions.map(a => (
          <div key={a.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:"0.5px solid #e5e7eb", opacity:0.55 }}>
            <input type="checkbox" checked={true} onChange={() => toggleAction(a.id)} style={{ width:15, height:15, cursor:"pointer", flexShrink:0 }} />
            <span style={{ flex:1, fontSize:13, color:"#6b7280", textDecoration:"line-through" }}>{a.text}</span>
            <button style={S.btnSm} onClick={() => deleteAction(a.id)}>✕</button>
          </div>
        ))}
      </>)}
  ...
  if (view === "add") return (
    <div style={S.page}>
      ...
      <div style={S.field}><label style={S.label}>Meeting notes</label><textarea style={{...S.input,height:120,resize:"vertical"}} value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Discussed AI strategy…" /></div>
      <div style={{borderTop:"0.5px solid #e5e7eb",paddingTop:16,marginBottom:16}}>
        <p style={{...S.label,marginBottom:8,fontSize:13,fontWeight:500,color:"#111"}}>Action item <span style={{fontWeight:400,color:"#6b7280"}}>(optional)</span></p>
        <div style={S.field}><input style={S.input} value={form.actionText} onChange={e=>setForm({...form,actionText:e.target.value})} placeholder="Send intro email, share deck…" /></div>
  ...
  if (view === "detail" && selected) {
    ...
    const urgentCount        = openContactActions.filter(a => a.due_date && (daysUntil(a.due_date) ?? 99) <= 1).length;
    const linkedinHref       = c.linkedin ? ((c.linkedin as string).startsWith('http') ? c.linkedin : 'https://' + c.linkedin) : undefined;

    const TabBtn = ({ id, label, badge }: { id: string; label: string; badge: number }) => (
      <button onClick={() => { setDetailTab(id); setFollowUpError(""); }}
        style={{ padding:"7px 18px", borderRadius:8, border: detailTab===id ? "0.5px solid #9ca3af" : "0.5px solid #d1d5db", background: detailTab===id ? "#fff" : "transparent", color: detailTab===id ? "#111" : "#6b7280", cursor:"pointer", fontSize:13, fontWeight: detailTab===id ? 500 : 400, display:"flex", alignItems:"center", gap:6 }}>
        {label}
        {badge > 0 && <span style={{ background:"#E24B4A", color:"#fff", borderRadius:10, fontSize:10, fontWeight:500, padding:"0 5px", minWidth:16, textAlign:"center", lineHeight:"16px" }}>{badge}</span>}
      </button>
    );
    ...
    return (
      <div style={S.page}>
        <div style={S.topBar}>
          <button style={S.btn} onClick={() => { setView("list"); setDetailTab("notes"); setFollowUpForm({text:"",dueDate:""}); setFollowUpError(""); }}>← Back</button>
          <button style={{...S.btn, color:"#dc2626"}} onClick={() => handleDeleteContact(c.id)}>Delete</button>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
          <Avatar name={c.name} size={52} />
          <div style={{ flex:1, minWidth:0 }}>
            <p style={{ margin:0, fontSize:18, fontWeight:500, color:"#111" }}>{c.name}</p>
            {(c.role||c.company) && <p style={{ margin:0, fontSize:13, color:"#6b7280" }}>{[c.role, c.company].filter(Boolean).join(" · ")}</p>}
            {linkedinHref && <a href={linkedinHref ?? undefined} target="_blank" rel="noopener noreferrer" style={{ fontSize:12, color:"#185FA5", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:4, marginTop:4 }}><LinkedInIcon /> LinkedIn</a>}
          </div>
        </div>
        <div style={{ display:"flex", gap:8, borderBottom:"0.5px solid #e5e7eb", paddingBottom:12, marginBottom:20 }}>
        ...
        {detailTab === "notes" && (
          <div>
            {c.date && <div style={{ marginBottom:14 }}><p style={S.label}>Date met</p><p style={{ margin:0, fontSize:14, color:"#111" }}>{c.date}</p></div>}
            {c.notes ? <div><p style={S.label}>Notes</p><p style={{ margin:0, fontSize:14, color:"#111", lineHeight:1.6, whiteSpace:"pre-wrap" }}>{c.notes}</p></div> : <p style={{ color:"#6b7280", fontSize:13 }}>No notes recorded.</p>}
            <p style={{ fontSize:11, color:"#9ca3af", marginTop:24 }}>Added {formatDate(c.created_at)}</p>
          </div>
        )}

        {detailTab === "followup" && (
          <div>
            <div style={{ background:"#f9fafb", border:"0.5px solid #e5e7eb", borderRadius:12, padding:"14px 16px", marginBottom:20 }}>
              <p style={{ ...S.label, fontWeight:500, marginBottom:10 }}>Add follow-up</p>
              <div style={S.field}><input style={S.input} value={followUpForm.text} onChange={e => setFollowUpForm({...followUpForm,text:e.target.value})} placeholder="Send intro email, share deck…" onKeyDown={e => e.key==="Enter" && handleAddFollowUp(c.id)} /></div>
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                <div style={{ flex:1 }}><label style={S.label}>Due date (optional)</label><input type="date" style={S.input} value={followUpForm.dueDate} onChange={e => setFollowUpForm({...followUpForm,dueDate:e.target.value})} /></div>
                <button style={{ ...S.btnPrimary, alignSelf:"flex-end" }} onClick={() => handleAddFollowUp(c.id)}>Add</button>
              </div>
              {followUpError && <p style={{ fontSize:12, color:"#dc2626", marginTop:8 }}>{followUpError}</p>}
            </div>
            {openContactActions.length === 0 && <p style={{ fontSize:13, color:"#6b7280" }}>No open follow-ups — add one above.</p>}
            {openContactActions.length > 0 && (<>
              <p style={S.section}>Open · {openContactActions.length}</p>
              {openContactActions.map(a => (
                <div key={a.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0", borderBottom:"0.5px solid #e5e7eb" }}>
                  <input type="checkbox" checked={false} onChange={() => toggleAction(a.id)} style={{ width:15, height:15, cursor:"pointer", flexShrink:0 }} />
                  <span style={{ flex:1, fontSize:13, color:"#111" }}>{a.text}</span>
                  {a.due_date && <DueBadge dateStr={a.due_date} />}
                  <button style={S.btnSm} onClick={() => deleteAction(a.id)}>✕</button>
                </div>
              ))}
            ...
            <div style={{ marginTop:24, paddingTop:16, borderTop:"0.5px solid #e5e7eb" }}>
              <button onClick={() => { setView("actions"); setDetailTab("notes"); }} style={{ ...S.btn, fontSize:12, color:"#6b7280" }}>View all action items →</button>
            </div>
          </div>
        )}
      </div>
